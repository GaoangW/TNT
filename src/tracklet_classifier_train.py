...
train_seqs = ['MOT17-02-SDP','MOT17-09-SDP','MOT17-10-SDP','MOT17-11-SDP','MOT17-13-SDP']
...
save_classifier_path = 'D:/Data/MOT/MOT17_train_det_crop/rand_forest_MOT17_SDP.pkl'
...
def h_err_pred(p,bbox,err_sigma,img_size):
    ...
    err_dist = np.absolute(h_pred[:,0]-h)/img_size[1]
    #import pdb; pdb.set_trace()
    '''
...
def extract_classifier_features():
    ...
    for n in range(len(train_seqs)):
        ...
        err_dist = np.zeros((len(temp_det),1))
        if F_use_flag[n]==1:
            ...
            for t in range(int(np.min(M[:,0])), int(np.max(M[:,0]))+1):
                ...
                #import pdb; pdb.set_trace()
                err_ratio[temp_det[:,0]==t,0], err_dist[temp_det[:,0]==t,0] = h_err_pred(GP_p,temp_det_bbox,err_sigma,img_size[n])
        else:
            GP_p = estimate_GP(M[:,1:5],err_sigma)
            #import pdb; pdb.set_trace()
            err_ratio[:,0], err_dist[:,0] = h_err_pred(GP_p,temp_det[:,1:5],err_sigma,img_size[n])
        err_ratio_h.append(err_ratio) 
        err_dist_h.append(err_dist)
    ...
    for n in range(len(train_seqs)):
        ...
        f = np.concatenate([f,err_dist_h[n]],axis=1)
